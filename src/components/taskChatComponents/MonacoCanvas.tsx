...
import type { MonacoFileItem } from "./TaskChat"

const MonacoCanvas = forwardRef((props: MonacoCanvasProps & { filesFromApi?: MonacoFileItem[] }, _ref) => {
  const {
    value,
    setValue,
    taskId,
  ...
  // Map TaskChat's filesFromApi -> Monaco's FileItem[]
  useEffect(() => {
    if (filesFromApi && filesFromApi.length > 0) {
      const mapped: FileItem[] = filesFromApi.map((f) => ({
        path: f.path,
        content: f.content,
        timestamp: (f as any).created_at || f.timestamp || new Date().toISOString(),
        // we keep isPdf/contentUrl on filesFromApi; we only use them for preview,
        // not for the plain file tree
      }))
      setFiles(mapped)
      setSelectedFile(mapped[0]?.path || null)

      const first = filesFromApi[0]
      if (first?.isPdf && first.contentUrl) {
        setPreviewIsPdf(true)
        setPreviewPdfUrl(first.contentUrl)
        setShowPreview(true)
      } else {
  ...
  const handleFileSelect = (filePath: string) => {
    ...
    // when selecting from tree, we assume text/code
    setPreviewIsPdf(false)
    setPreviewPdfUrl(null)
  ...
  useEffect(() => {
    if (executeTaskRef) executeTaskRef.current = handleExecuteTask
  ...
  const handleTogglePreview = () => {
    // toggling preview when we already know if it's pdf or html
    if (!showPreview && !previewIsPdf) {
  ...
  }, [currentFileContent, showPreview, previewIsPdf])

  const handleSyncWithGitHub = useCallback(() => {
    if (onSyncWithGitHub) onSyncWithGitHub()
  }, [onSyncWithGitHub])
